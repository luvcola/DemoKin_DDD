...
author: |
  | Instructor: Diego Alburez-Gutierrez (Max Planck Institute for Demographic Research)
date: "Workshop 'The Formal Demography of Kinship: Theory and Application' - Dutch Demography Day; Nov 16 2022"
output:
  github_document:
    pandoc_args: --webtex
    toc: true
...
Install the development version from GitHub (could take ~1 minute). We made changes to the `DemoKin` package ahead of this workshop If you had already installed the package, please uninstall it and and install it again.
...
This is the Swedish data we will be using:
...
- **U** numeric. A vector (atomic) or matrix with probabilities (or survival ratios, or transition between age class in a more general perspective) with rows as ages (and columns as years in case of matrix, being the name of each col the year).
- **f** numeric. Same as U but for fertility rates.
- **time_invariant** logical. Constant assumption for a given year rates. Default TRUE.
...
This data frame contains expected kin counts by year (or cohort in case `time_invariant` = FALSE), age of Focal and age of kin. 
...
> Note: we are working in a time invariant framework. You can think if the results as analogous to life expectancy: expected years of life for a synthetic cohort experiencing observed period mortality rates.
